...
export async function getAdminSession() {
  ...
  const roleRecords = session.user.roles.map((entry) => entry.role);
  const roleTypes = roleRecords.map((r) => r.type);

  // Super Admin bypass: Grant all known permissions
  const isSuperAdmin = roleTypes.includes("SUPER_ADMIN");
  let permissions = [];

  if (isSuperAdmin) {
    // If super admin, effectively they have everything. 
    // We can return a wildcard or fetch all from catalog.
    // For safety in frontend checks, let's include all known + a wildcard.
    // You might also handle this via a recursive function or just a "*" check in guard.
    // simpler to just give all permissions from the catalog + "*"
    const { ROLE_CATALOG } = await import("./roleCatalog");
    const allCatalogPermissions = ROLE_CATALOG.flatMap(r => r.permissions);
    permissions = Array.from(new Set([...allCatalogPermissions, "*"]));
  } else {
    // Combine legislative permissions (from catalog) and explicit DB permissions
    const derived = derivePermissionsFromRoles(roleTypes);

    // permissions from Custom Roles
    const customPermissions = roleRecords.flatMap(r => {
      if (Array.isArray(r.permissions)) return r.permissions;
      return [];
    });

    permissions = Array.from(new Set([...derived, ...customPermissions]));
  }
