...
export const attachmentSchema = attachmentBase.refine((data) => data.mediaId || data.mediaUrl, {
    message: "A file must be selected",
    path: ["mediaId"]
});

export const updateAttachmentSchema = attachmentBase.pick({ label: true, mediaId: true, mediaUrl: true }).extend({
    id: z.string().min(1),
...
export const updateServiceDetailSchema = serviceDetailSchema.omit({ serviceCardId: true }).extend({
    id: z.string().min(1),
    serviceCardId: z.string().optional(),
});

const serviceResourceBase = z.object({
    categoryId: z.string().min(1, "Category is required"),
    title: z.string().min(1, "Title is required"),
    description: z.string().nullable().optional(),
...
export const serviceResourceSchema = serviceResourceBase.refine((data) => data.mediaId || data.mediaUrl || data.externalUrl, {
    message: "Must provide either a file or an external URL",
    path: ["mediaId"]
});

export const updateServiceResourceSchema = serviceResourceBase.extend({
    id: z.string().min(1),
