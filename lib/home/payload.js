...
async function fetchHomeHero() {
  try {
    ...
    if (!page) return DEFAULT_HERO;

    const heroSection = page.sections.find((s) => s.type === "HERO");
    if (!heroSection || !heroSection.content) return DEFAULT_HERO;

    const c = heroSection.content;
    return {
      eyebrow: c.eyebrow || DEFAULT_HERO.eyebrow,
      title: c.title || DEFAULT_HERO.title,
      subtitle: c.subtitle || DEFAULT_HERO.subtitle,
      ctaLabel: c.ctaLabel || DEFAULT_HERO.ctaLabel,
      ctaHref: c.ctaHref || DEFAULT_HERO.ctaHref,
      backgroundImage: c.backgroundImage || DEFAULT_HERO.backgroundImage,
      // If you have media objects in content, map them here if needed
    };
  } catch (error) {
    console.warn("Failed to fetch home hero:", error);
    return DEFAULT_HERO;
  }
}
...
export async function buildHomePayload() {
  ...
  try {
    [projectHighlights, leadershipMembers, achievements, counterStats, workflowSteps, mediaItems, faqsData, waterTodayUpdates, heroData] = await Promise.all([
