...
export async function buildHomePayload(lang = 'en') {
  ...
  const achievementsPayload = achievements.length
    ? achievements.map((entry) => ({
      id: entry.id,
      title: lang === 'ur' && entry.titleUr ? entry.titleUr : entry.title,
      summary: lang === 'ur' && entry.summaryUr ? entry.summaryUr : entry.summary,
      metric: entry.metric,
      icon: entry.iconKey,
    }))
  ...
  const counters = counterStats.length
    ? counterStats.map((stat) => ({
      id: stat.id,
      label: lang === 'ur' && stat.labelUr ? stat.labelUr : stat.label,
      value: stat.value,
      suffix: stat.suffix || "",
    }))
    : fallbackCounters();
  ...
  const workflow = workflowSteps.length
    ? workflowSteps.map((step, index) => ({
      ...
      originalId: step.id,
      title: lang === 'ur' && step.titleUr ? step.titleUr : step.title,
      summary: lang === 'ur' && step.summaryUr ? step.summaryUr : step.summary,
      theme: step.theme || null,
      order: step.order,
    }))
    : fallbackWorkflow();
  ...
  const faqs = faqsData.length
    ? faqsData.map((faq) => ({
      id: faq.id,
      question: lang === 'ur' && faq.questionUr ? faq.questionUr : faq.question,
      answer: lang === 'ur' && faq.answerUr ? faq.answerUr : faq.answer,
      category: lang === 'ur' && faq.category?.titleUr ? faq.category.titleUr : (faq.category?.title || "General"),
    }))
    : fallbackFaqs();
