...
/**
 * Contact page rebuilt to avoid hook-order warnings and ensure Urdu/English swap.
 * All hooks run on every render, no conditional hook paths.
 */
export default function Contact() {
  ...
  // Derived data
  const { hero, channels = [], offices = [] } = useMemo(() => {
    const defaultHero = {
      title: "CONTACT KW&SC",
  ...
  // Normalized data usage (API handles localization)
  const displayAddress = primaryOffice?.address || fallbackAddress;
  const displayHeroTitle = hero.title;
  const displayHeroSubtitle = hero.subtitle;
  ...
  // Early returns after hooks/state are declared
          ...
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <div className="bg-white p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12 2xl:p-14 rounded-2xl sm:rounded-2xl md:rounded-3xl lg:rounded-3xl shadow-2xl border border-gray-100">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-5 lg:gap-6 xl:gap-7 2xl:gap-8">
                ...
                <div className="md:col-span-2 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4 md:gap-5 lg:gap-6 mt-3 sm:mt-4 md:mt-6 lg:mt-8">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`px-6 sm:px-8 md:px-10 lg:px-12 xl:px-14 py-2 sm:py-2.5 md:py-3 lg:py-3 text-xs sm:text-sm md:text-base lg:text-lg text-white font-bold rounded-lg sm:rounded-xl md:rounded-xl lg:rounded-xl transition-all shadow-lg w-full sm:w-auto ${isSubmitting ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-600 to-blue-800 hover:shadow-blue-500/50"
                      }`}
              ...
              <p className="mt-6 sm:mt-7 md:mt-8 lg:mt-10 text-center text-xs sm:text-xs md:text-sm lg:text-base text-gray-500 font-medium border-t pt-3 sm:pt-4 md:pt-5 lg:pt-6">
                {t("NOTE: For Further Information, please contact us on our helpline:")}{" "}
                <a href="tel:+92021111597200" className="text-blue-600 hover:underline">
                  111-597-200
                </a>
              </p>
            </div>
