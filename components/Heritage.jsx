...
export default function Heritage({ heading, body }) {
  ...
  if (hasCustomHTML) {
    ...
    const paragraphs = pMatches
      ? pMatches.map((p) => p.replace(/<[^>]+>/g, "").trim())
      : [];
              ...
              {extractedContent ? (
                ...
                <div className="text-slate-300 text-base sm:text-lg md:text-xl leading-relaxed space-y-4 md:space-y-6">
                  <p>
                    ...
                    efficient sewerage services to all residents of Karachi. Our
                    commitment to excellence and innovation enables us to
                    deliver reliable solutions tailored to the city's needs.
                  </p>
                  <p>
                    Our mission is to transform KW&SC into a customer-centric,
                    financially autonomous, and technologically advanced water
                    and sewerage utility. We leverage sustainable practices to
                    ensure long-term success and contribute to Karachi's water
                    security and sanitation goals.
                  </p>
                </div>
              )}
            </Fade>
