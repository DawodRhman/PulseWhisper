...
const API_ROUTE = "/api/admin/tenders";
const EMPTY_PAYLOAD = { tenders: [], categories: [] };
...
export function useAdminTenders() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(parseErrorPayload(payload, "Failed to load tenders"));
      }
      setData(payload?.data || EMPTY_PAYLOAD);
